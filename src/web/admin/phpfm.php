...
// +--------------------------------------------------
    ...
    if(! (isset($_SESSION[$OJ_NAME.'_'.'administrator']) || isset($_SESSION[$OJ_NAME.'_'."p".$pid])) ){
        echo "No Privilege.<br>관리자가 아니거나 이 문제의 원작자가 아니므로 데이터를 관리할 수 없습니다.";
        exit(0)    ;
    }
    ...
    if(isset($_GET['generate'])){
		...
		echo "<script>window.setTimeout('resolveIDs()',5000);</script>";
           	trigger_judge($insert_id);     // moved to my_func.inc.php
                ...
                echo "Main.c 또는 Main.cc를 찾을 수 없어 빈 파일 Gen.py와 Main.cc를 자동 생성합니다.";
                touch("Gen.py");
                touch("Main.cc");
            }
    }
...
function et($tag){
    ...
    $cn['To'] = '→';
    $cn['Destination'] = '대상';
    $cn['Configurations'] = '설정';
    $cn['JSError'] = 'JS 스크립트 오류';
    $cn['NoSel'] = '파일 또는 폴더를 선택하세요';
    $cn['SelDir'] = '왼쪽 폴더 트리에서 대상 폴더를 선택하세요';
    $cn['TypeDir'] = '폴더명을 입력하세요';
    $cn['TypeArq'] = '파일명을 입력하세요\\n    입력파일: *.in         출력파일:*.out\\n    freopen 파일명 지정 ↴          소스코드 파일명 ↴\\n                  input.name output.name            solution.name \\n    채점 설정:judge.conf \\n    테스트 입력 생성기:Gen.py    \\n    표준 코드:Main.c/Main.cc\\n자세한 내용은 hustoj.com ';
    $cn['TypeCmd'] = '명령어를 입력하세요';
    $cn['TypeArqComp'] = '파일명을 입력하세요.확장자에 따라 압축 형식이 결정됩니다.\\n예:file.zip, file.tar, file.bzip, file.gzip';
    $cn['RemSel'] = '선택 항목 삭제';
    $cn['NoDestDir'] = '대상 폴더를 선택하세요';
    $cn['DestEqOrig'] = '대상 폴더와 원본 폴더가 동일합니다';
    $cn['InvalidDest'] = '잘못된 대상 폴더';
    $cn['NoNewPerm'] = '새 권한이 설정되지 않았습니다';
    $cn['CopyTo'] = '복사 위치';
    $cn['MoveTo'] = '이동 위치';
    $cn['AlterPermTo'] = '권한 변경';
    $cn['ConfExec'] = '실행 확인';
    $cn['ConfRem'] = '삭제 확인';
    $cn['EmptyDir'] = '빈 폴더';
    $cn['IOError'] = 'I/O 오류';
    $cn['FileMan'] = '파일 관리자';
    $cn['TypePass'] = '비밀번호를 입력하세요';
    $cn['InvPass'] = '비밀번호 오류';
    $cn['ReadDenied'] = '접근 거부됨';
    $cn['FileNotFound'] = '파일을 찾을 수 없습니다';
    $cn['AutoClose'] = '완료 후 닫기';
    $cn['OutDocRoot'] = '파일이 루트 디렉토리에 없습니다';
    $cn['NoCmd'] = '오류: 잘못된 명령어';
    $cn['ConfTrySave'] = '파일에 쓸 수 없습니다.\\n다른 위치에 저장해보세요';
    $cn['ConfSaved'] = '설정이 저장되었습니다';
    $cn['PassSaved'] = '비밀번호가 저장되었습니다';
    $cn['FileDirExists'] = '파일 또는 폴더가 이미 존재합니다';
    $cn['NoPhpinfo'] = 'phpinfo 함수가 비활성화됨';
    $cn['NoReturn'] = '반환값 없음';
    $cn['FileSent'] = '파일 전송 완료';
    $cn['SpaceLimReached'] = '용량 한도 초과';
    $cn['InvExt'] = '허용되지 않는 확장자';
    $cn['FileNoOverw'] = '파일을 덮어쓸 수 없습니다';
    $cn['FileOverw'] = '덮어쓰기';
    $cn['FileIgnored'] = '무시됨';
    $cn['ChkVer'] = 'sf.net에서 새 버전 확인';
    $cn['ChkVerAvailable'] = '새 버전이 있습니다. 여기를 클릭하여 다운로드하세요!';
    $cn['ChkVerNotAvailable'] = '현재 새 버전이 없습니다. :(';
    $cn['ChkVerError'] = '연결 오류.';
    $cn['Website'] = '사이트';
    $cn['SendingForm'] = '파일 전송 중, 잠시만 기다려주세요';
    $cn['NoFileSel'] = '파일을 선택하세요';
    $cn['SelAll'] = '전체선택';
    $cn['SelNone'] = '선택해제';
    $cn['SelInverse'] = '선택반전';
    $cn['Selected_s'] = '선택됨';
    $cn['Total'] = '합계';
    $cn['Partition'] = '디스크 파티션';
    $cn['RenderTime'] = '페이지 로딩 시간';
    $cn['Seconds'] = '초';
    $cn['ErrorReport'] = '오류 보고';
    $cn['Random-data'] = '랜덤 테스트 데이터 생성기';
    $cn['GenerateOut'] = 'Gen.py+Main.c로 테스트 데이터 생성';
    $cn['Ans2out'] = '파일명 자동 수정';
    $cn['IOFilename'] = '입출력 파일명 지정';
    $cn['SolutionFilename'] = 'NOIP 제출 코드 파일명 지정';
...
function tips($filename) {
    ...
    if ($lower === 'main.c') {
        ...
        return '테스트 입력 데이터 생성기 Python 스크립트';
    } elseif ($lower === 'solution.name') {
        return '파일 업로드 방식으로 답안 제출, 규정된 파일명 포함';
    } elseif ($lower === 'input.name') {
        return '파일 방식 입력, 규정된 파일명 포함';
    } elseif ($lower === 'output.name') {
        return '파일 방식 출력, 규정된 파일명 포함';
    } elseif ($lower === 'judge.conf') {
        return '채점 파라미터 설정, hustoj.com 참조';
    }
    ...
    $patterns = [
        '/^template\.([a-z0-9_]+)$/i' => '답안 템플릿',
        '/^prepend\.([a-z0-9_]+)$/i'  => '앞부분 추가 소스코드',
        '/^append\.([a-z0-9_]+)$/i'   => '뒷부분 추가 소스코드'
    ];
...
function dir_list_form() {
    ...
    if ($opdir = @opendir($current_dir)) {
        ...
        function test_prompt(arg){
            ...
            if (arg == 1){
                ...
                document.form_action.cmd_arg.value = prompt('".et('IOFilename').",확장자 제외, 예: tree');
            } else if (arg == 22){
	...
	if(!$OJ_SaaS_ENABLE)$out.="<input class='btn-success' type=button onclick=\"generate()\" value=\"".et('GenerateOut')."\" title='직접 작성이 어려우면 AI 도움을 받을 수 있습니다'>";
	$out.="<input type=button class='btn-warning' onclick=\"ans2out()\" value=\"".et('Ans2out')."\" title='숫자 앞에 0 자동추가 정렬 + ans를 out으로 변환' >";
	$out.="<input type=button class='btn-primary' onclick=\"test_prompt(21)\" value=\"".et('IOFilename')."\" title='CCF 대회에서 각 문제에 영문명이 있습니다'>";
	$out.="<input type=button class='btn-danger' onclick=\"test_prompt(22)\" value=\"".et('SolutionFilename')."\" title='CCF 대회에서 요구하는 tree.cpp 같은 형식'>";
	if(isset($_GET['pid'])){
                $pid=intval($_GET['pid']);
        ...
        if($entry_count){
            ...
            $out .= "<span title='일반적으로 영문 파일명을 권장하며, 같은 이름의 .in .out 파일이 한 쌍입니다.
.ans 확장자는 지원되지 않습니다. 업로드 전 Windows 명령줄에서 ren *.ans *.out으로 변경하세요.
파일명에 대괄호[]로 점수를 표기할 수 있습니다.예: test01[20].in / test01[20].out의 배점은 20점이며, 표기 없는 파일은 10점으로 계산됩니다.
시스템은 전체 파일의 총점과 실행 점수를 기반으로 통과율(pass_rate)을 기록합니다.
모든 파일에 점수 표기가 없는 문제는 각 파일 쌍이 동일 배점으로 처리됩니다.
채점 시 모든 .in 파일의 사전순으로 평가합니다. 따라서 test10은 test2보다 먼저, test02보다 나중에 평가됩니다.
subtask 명명규칙: 점수 앞에 영문 마침표(.)를 추가하면 마침표 앞 부분이 subtask 이름이 되며, 같은 이름의 테스트 데이터가 하나의 그룹으로 묶입니다.
예: subtask1.1[10].in/out 과 subtask1.2[20].in/out 은 하나의 subtask 그룹이며, 모두 통과해야 총 30점을 받습니다.
subtask 이름은 숫자가 아니어도 됩니다. 예: TaskA.a[20].in/out 과 TaskA.b[20].in/out 도 하나의 그룹으로, 총 40점입니다.
subtask 문제에서 다른 데이터와 묶이지 않는 파일은 독립 그룹으로 취급됩니다. 예: big[30].in/out을 위 두 예시와 함께 사용하면 총 100점입니다.' >
               파일 명명 규칙: .in/.out 쌍 대응, []로 점수 표기, .으로 subtask 구분</span>
                </nobr></td>
                </tr>
				<tr>
            ...
            foreach ($entry_list as $ind=>$dir_entry) {
                ...
                if ($dir_entry["type"]=="dir"){
                    ...
                    $file_out[$file_count][] = "<td><nobr>".($dir_entry["sizet"]=='0 bytes'?"<span class='label label-danger'>빈 파일</span>":$dir_entry["sizet"])."</nobr></td>";
...
function upload_form(){
    ...
    if (count($_FILES)==0){
        ...
        for ($x=0;$x<$num_uploads;$x++){
            echo "<tr><td align=right><b>".et('File').":<td><nobr><input type=\"file\" multiple name=\"files[]\"   title='한 번에 여러 파일을 선택하여 업로드할 수 있습니다'   ></nobr>";
        ...
        <tr><td colspan=2>한 번에 여러 파일을 선택하여 업로드할 수 있습니다<br> zip file can be decompressed on the server later. 
			  just don't add dirs,please<br>
			  zip 파일을 업로드한 후 decompress를 클릭하여 압축을 해제할 수 있습니다. zip 파일에 하위 폴더를 포함하지 마세요.
	</td></tr>
        </form>
        </table>
        <script language=\"Javascript\" type=\"text/javascript\">
...
function edit_file_form(){
   ...
   // echo "[$filename]";
	...
	if(str_ends_with($filename,".in") || $filename=="Gen.py" || str_starts_with($filename,"Main.")  ) 
		echo "<input id='ai_bt' class='btn btn-primary' type=button value='AI 생성' onclick='ai_gen(\"".$filename."\")' > *AI가 생성한 코드는 직접 확인 후 실행하세요. 예상대로 동작하지 않으면 다시 생성하세요* ";
