...
// $DB_PASS="eqAHtdiYFUvCWbABBMqurEIKezlZF0"; 里面放的密码是安装是随机生成的，请不要泄露，最好也不要随意修改为低强度的其他密码，您在linux命令行可以用sudo mysql直接以root身份登录mysql。
// connect db 
static 	$DB_HOST="localhost";  //数据库服务器ip或域名
static 	$DB_NAME="jol";   //数据库名
static 	$DB_USER="hustoj";  //数据库账户
static 	$DB_PASS="eqAHtdiYFUvCWbABBMqurEIKezlZF0";  //数据库密码
static 	$OJ_NAME="Codit";  //左上角显示的系统名称, 尽量简洁，避免用中文和空格，如需中文、长字符串、图片，可以直接修改template/syzoj/header.php
static 	$OJ_HOME="./";    //主页目录
...
static  $OJ_MARK=""; // "mark" 显示正确得分， "percent" 显示错误比率
...
static  $OJ_TEMPLATE="bs3"; //使用的默认模板,template目录下的每个子目录都是一个模板, [bs3 mdui sweet syzoj sidebar bshark] work with discuss3
...
static  $OJ_FRIENDLY_LEVEL=5; //系统友好级别，暂定0-9级，级别越高越傻瓜，系统易用度高的同时将降低安全性，仅供非专业用途，造成泄题、抄袭概不负责。
static  $OJ_FREE_PRACTICE=false; //自由练习，不受比赛作业用题限制
static  $OJ_SUBMIT_COOLDOWN_TIME=5; //提交冷却时间，连续两次提交的最小间隔，单位秒。
static  $OJ_POISON_BOT_COUNT=50; //给机器人账号投毒的起始AC数。例如设为10，则认为一个账号对某个题提交了10次正确之后还在提交，就是机器人行为，开始给出随机答复。
