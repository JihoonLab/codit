...
if(!($OJ_TEMPLATE=="syzoj" || $OJ_TEMPLATE=="sidebar")) {
    $view_news = "";
    $sql = "SELECT news_id, title, time, importance FROM `news` WHERE `defunct`!='Y' AND `title` NOT LIKE '%.ko' AND `title` NOT LIKE 'home.%' AND `title` NOT LIKE 'privacy.%' AND `title` NOT LIKE 'faqs.%' ORDER BY importance DESC, time DESC LIMIT 6";
...
$view_apc_info = "";
$last_1000_id = 0;
$last_1000_id = mysql_query_cache("select min(solution_id) id from solution where in_date >= NOW() - INTERVAL 8 DAY union select GREATEST(0, CAST(max(solution_id) AS SIGNED)-1000) id from solution order by id desc limit 1");
